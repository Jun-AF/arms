...
class UserController extends Controller {
    ...
    public function store(Request $request) {
        $this->is_admin(); // check apakah role user adalah admin
        $validate = $request->validateWithBag("error", ["name" => ["required", "string", "min:1", "max:25", "regex:/[A-Za-z ]/"], "email" => ["required", "string", "max:25", "unique:users,email", "regex:/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/"], "password" => ["required", "string", "min:6", "max:255", "regex:/([A-Za-z0-9_!*])/"]]);
        $user = null;
        DB::beginTransaction();
        try {
            $user = User::create(["name" => $request->name, "email" => $request->email, "password" => Hash::make($request->password), "created_at" => now() ]);
            DB::commit();
        }
    ...
    public function update(Request $request) {
        $this->is_admin(); // check apakah role user adalah admin
        $validate = $request->validate(['id' => ["required", "integer", "exists:App\Model\User,id"], "name" => ["required", "string", "min:1", "max:25", "regex:/[A-Za-z ]/"], "email" => ["required", "string", "max:25", "unique:users,email,$request->id", "regex:/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/"]]);
        $user = null;
        DB::beginTransaction();
        try {
    ...
    public function updatePassword(Request $request) {
        $this->is_admin(); // check apakah role user adalah admin
        $validate = $request->validate(['id' => ["required", "integer", "exists:App\Model\User,id"], "password" => ["required", "string", "min:6", "max:255", "regex:/([A-Za-z0-9_!*])/"], ]);
        $user = null;
        DB::beginTransaction();
        try {
            $user = User::where("id", $request->id)->lockForUpdate()->first(); // lock for update prevent the race condition
    ...
    public function delete(Request $request) {
        ...
        $this->toastNotification("Success", $user->name . " has deleted");
        $condition = $this->getCondition();
        $notif = $this->getNotif();
        return redirect('admin')->with(["condition" => $condition, "notif" => $notif]);
    }
    ...
    public function userProfileUpdate(Request $request) {
        $validate = $request->validate(["name" => ["required", "string", "min:1", "max:25", "regex:/[A-Za-z ]/"], "email" => ["required", "string", "max:25", "unique:users,email,$request->id", "regex:/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/"]]);
        DB::beginTransaction();
        try {
            $user = User::where("id", $request->id)->lockForUpdate()->first(); // lock for update prevent the race condition
            $user->name = $request->name;
    ...
    public function userActivity() {
        $activities = $this->activity(); // mengambil record aktivitas user
        $data = DB::table("activities as act")->select("act.id as act_id", "act.actor_id", "users.id", "users.name as actor", "act.token", "act.message", "act.type", "act.is_read", "act.created_at")->leftJoin("users", "users.id", "act.actor_id")->get();
    ...
    public function readAll() {
        DB::beginTransaction();
        try {
            DB::table('activities')->update(['is_read' => true]);
            DB::commit();
        }
        catch(QueryException $err) {
            DB::rollBack();
    ...
    public function getActivity($id) {
        ...
        DB::beginTransaction();
        try {
            $read = Activity::where('id', $id)->update(['is_read' => true, ]);
            DB::commit();
        }
        catch(QueryException $err) {
            DB::rollBack();
        ...
        $data = DB::table("activities as act")->select("act.id as act_id", "act.actor_id", "users.id", "users.name as actor", "act.token", "act.message", "act.type", "act.created_at")->leftJoin("users", "users.id", "act.actor_id")->where("act.id", $id)->get();
        $activities = $this->activity(); // mengambil record aktivitas user
