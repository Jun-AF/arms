				...
				<form action="{{ route('history.update') }}" method="POST"> 
					...
					<div class="row mb-2 justify-content-center">
						...
						<div class="col-md col-sm align-items-center">
							<select class="form-control @error('asset_id') is-invalid @enderror" name="asset_id"> @foreach ($assets as $ast) <option value="{{ $ast->id }}" {{ ($history->asset_id == $ast->id) ? 'selected':'' }}>{{ $ast->asset_name .' - '. $ast->sn }}</option> @endforeach </select>
						</div>
					</div>
					<div class="row mb-2 justify-content-center">
						<div class="col-md-3 col-sm-2 align-items-center text-right">
